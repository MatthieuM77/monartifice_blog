# Valider septembre 2026 — état et checklist

**17 publications · 44 slides · 8 carrousels.**
Tout est écrit et maquetté. Il reste **9 photos** et **4 relectures**.

---

## 1. Ce qui est prêt

| Élément | État |
|---|---|
| Stratégie, piliers, cadence | ✅ |
| Calendrier des 17 publications | ✅ |
| **Textes Instagram** — 17/17 | ✅ |
| **Textes Facebook** — 17/17, réécrits, pas des copies | ✅ |
| **Hashtags** — 14 par post Instagram, 3 par post Facebook | ✅ |
| Stories associées décrites | ✅ |
| **44 slides maquettées**, aux couleurs et au logo de la marque | ✅ |
| 5 photos intégrées | ✅ |
| Import Canva | ✅ — **un design par publication** |

## 2. Les 8 carrousels

| # | Date | Publication | Slides |
|---|---|---|---|
| 04 | 07/09 | Faut-il prévenir la mairie ? | 5 |
| 07 | 13/09 | Le méchage | 5 |
| 08 | 14/09 | 8 mètres ou 25 mètres ? | 5 |
| 09 | 16/09 | La mise d'inflammateur | 4 |
| 12 | 21/09 | F2 ou F3 ? | 4 |
| 13 | 23/09 | Le tableau de tir | 4 |
| 16 | 28/09 | Et s'il pleut ? | 4 |
| 17 | 30/09 | Ce qu'il reste après | 4 |

Les 9 autres publications sont des visuels uniques : Reels, posts simples, sondage, appel UGC.

---

## 3. ⏳ Ce qui manque — 9 photos

Chaque ligne bloque une slide précise. Les codes sont inscrits dans les cadres réservés
des maquettes Canva : vous voyez directement où chaque photo doit venir.

| Plan | Publication | Slide | Ce qu'il faut photographier | Cadrage |
|---|---|---|---|---|
| **I1** | 11/09 — Le moment où tout le monde lève la tête | 1/1 | Le public de dos, têtes levées, éclairé par le feu | Depuis le côté public, au moment du tir |
| **A2** | 13/09 — Le méchage | 3/5 | Le détail d'un raccord de mèche | Macro, sur le dispositif méché |
| **A3** | 13/09 — Le méchage | 4/5 | Le dispositif méché en plan large, prêt à tirer | Trois-quarts, avant le tir |
| **B2** | 16/09 — La mise d'inflammateur | 2/4 | Le câblage des inflammateurs | Serré sur les fils et leurs couleurs |
| **H3** | 18/09 — Il a dit oui. Le ciel aussi. | 1/1 | Un départ vertical serré, plein cadre | Depuis le poste de tir |
| **D2** | 23/09 — Le tableau de tir | 2/4 | Les lignes numérotées du tableau de tir | Macro sur le pupitre |
| **H2** | 25/09 — Les mariages de septembre | 1/1 | Le feu en plan large avec le lieu dans le cadre | Cadrer large, ne pas serrer sur le ciel |
| **F1** | 30/09 — Ce qu'il reste après | 1/4 | Le site vide après le tir, dispositif éteint | Lumière froide, fin d'intervention |
| **F2** | 30/09 — Ce qu'il reste après | 2/4 | Le ramassage des reliquats à la lampe | Gros plan mains + lampe |

### Priorité si le temps manque

1. **I1** — irrattrapable. Ne s'obtient qu'au moment du tir, depuis le côté public.
   Si vous êtes tous les deux au poste, confiez un téléphone à quelqu'un.
2. **H3** et **H2** — le feu lui-même. Deux cadrages différents, ne prenez pas que du serré.
3. **A2, A3, B2, D2** — les macros de chantier. Faciles, à faire pendant le montage.
4. **F1, F2** — après le tir. C'est celles qu'on oublie parce qu'on est fatigué.

---

## 4. ⏳ Ce qui manque — 4 relectures

Ces quatre publications portent des affirmations réglementaires ou sécuritaires.
Elles sont formulées en principes généraux et renvoient à la notice du produit,
jamais en conseil juridique. **À relire avant publication.**

| Date | Publication | Ce qu'il faut vérifier |
|---|---|---|
| 07/09 | Faut-il prévenir la mairie ? | Que les trois critères annoncés sont les bons |
| 14/09 | 8 mètres ou 25 mètres ? | Que F2 → 8 m et F3 → 25 m vaut bien pour **tout** le catalogue |
| 21/09 | F2 ou F3 ? | Qu'aucune obligation légale précise n'est énoncée sans source |
| 28/09 | Et s'il pleut ? | Qu'aucun seuil chiffré de vent n'a été ajouté |

---

## 5. Trois décisions

**A. La formulation Ciels en Fête.** Le logo porte déjà « by Ciels en Fête », donc le lien
est affiché en permanence. Le post épinglé du 9 septembre l'explicite. Validez la phrase ou
donnez-moi la vôtre.

**B. Le compte Instagram et la page Facebook existent-ils ?** Si non, à créer avant le 2/09.
Nom, bio, photo de profil, lien. Je peux rédiger la bio.

**C. Les vidéos produit.** 27 références ont un ID YouTube dans la base. Si vous avez le droit
de les réutiliser, elles règlent une partie du besoin en images de feu pour octobre. À vérifier
auprès du fournisseur.

---

## 6. Ce que je fais dès réception

1. Vous déposez les photos dans `social/photos/2026-09/`, nommées par code de plan
   (`I1-public-tetes-levees.jpg`, `A2-raccord-macro.jpg`…).
2. Je les intègre aux 9 slides, je réajuste les cadrages et les voiles.
3. Je régénère et je réimporte dans Canva.
4. **Septembre est publiable.**

Délai de mon côté : immédiat. Le seul chemin critique, c'est le feu du 5 septembre.
